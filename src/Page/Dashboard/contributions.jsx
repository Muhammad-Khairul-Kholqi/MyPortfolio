...
const Contributions = () => {
    ...
    if (loading) {
        return (
            <div className="text-center">
                <div role="status">
                    <svg aria-hidden="true" className="inline w-8 h-8 text-gray-200 animate-spin dark:text-gray-400 fill-green-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    const contributionData = [
        ...
        { label: "Average", value: isNaN(average) ? 'N/A' : average.toFixed(2) }
    ];
        ...
        <div>
            ...
            <div className="summary flex flex-wrap justify-start gap-[20px] mt-[10px]">
                {contributionData.map((data, index) => (
                    <div key={index} className="bg-[#F5F5F5] p-[10px] w-[220px] text-start rounded-[10px] border">
                        <p className="text-[13px]">{data.label}</p>
                        <p className="text-green-500 text-[25px]">{data.value}</p>
                    </div>
                ))}
            </div>
        </div>
